...
OUT = Path(r"Z:\Megha_Kattimani\Full_pipeline test\mezzo_H2A_phase2")
...
CONDITIONS = ["E_ctrl", "P_ctrl", "P_Activin_3-5hpf"]
HPF_START = 7.0
HPF_INTERVAL = 698.8316040039062 / 3600.0   # ~0.1941 hpf (~11.65 min)

# --- thresholds (locked in with user) ---
GLOBAL_THR_PERCENTILE = 99
GLOBAL_THR_FRAMES = (0, 10)       # ~7.0-8.9 hpf baseline (pre-induction)
# P_ctrl ONLY: E_ctrl embryos at 7-9 hpf are mid-gastrulation and already
# express endogenous mezzo at the margin, so they are not a mezzo-negative
# baseline. P_ctrl pescoids pre-induction (p50~0.6) are the true baseline.
GLOBAL_THR_CONDITIONS = ["P_ctrl"]
...
LINK_IOU = 0.30
ANALYSIS_MAX_HPF = 16.0           # cap analysis at 16 hpf (pre-disintegration)
PEAK_WINDOW_HPF = (7.0, 16.0)     # full meaningful biology window; phenotype counts any
                                  # pole present in this window. Before/at/after-peak
                                  # dynamics inside it are reported by Phase 3.
...
if __name__ == "__main__":
    ...
    p.add_argument("--sanity", action="store_true",
                   help="Run on S35 + S15 only for sanity check")
