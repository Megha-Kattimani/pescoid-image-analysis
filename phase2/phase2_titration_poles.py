...
Adapted from phase2_poles.py: new paths, E/P condition set, S-numbered
pescoids, 93-frame timing (7 hpf start, ~11.65 min interval).

Usage
-----
    python phase2_h2a_poles.py --sanity            # S35 + S15 sanity check
    python phase2_h2a_poles.py --all               # full run on QC-pass pescoids
"""
...
# Paths
# ---------------------------------------------------------------------------
PHASE1 = Path(r"Z:\Megha_Kattimani\Full_pipeline test\mezzo_titration_phase1")
PHASE1_QC = PHASE1 / "phase1_QC.csv"
OUT = Path(r"Z:\Megha_Kattimani\Full_pipeline test\mezzo_titration_phase2")
...
GLOBAL_THR_FRAMES = (0, 6)        # ~8.0-11 hpf baseline (pre-induction)
# Pctrl ONLY is the mezzo-negative baseline (no Activin). Embryos express
# endogenous gastrulation mezzo, so they are excluded from the threshold calc.
GLOBAL_THR_CONDITIONS = ["Pctrl"]

AREA_POLE_PX = 500
# Small bright poles: clusters smaller than AREA_POLE_PX but with high per-pixel
# mezzo intensity (locally enriched mezzo expression) also count as poles.
# Captures small-but-bright mezzo+ poles that the simple area rule misses.
SMALL_POLE_AREA_MIN = 150
SMALL_POLE_INTENSITY_MIN = 1500  # peak bg-sub mean intensity, per-pixel
AREA_OC_PX = 100
# Persistence is in FRAMES; this experiment is 30 min/frame (vs 11.65 min for
# H2A), so frame counts are lowered to keep a comparable real-time requirement
# (~1.5 h pole persistence, ~2.5 h for split-daughter independence).
POLE_PERSISTENCE_MIN = 3
OC_PERSISTENCE_MIN = 2
SPLIT_DAUGHTER_PERSISTENCE = 5
PERIPHERAL_OFFSET_FRAC = 0.30
LINK_IOU = 0.30
ANALYSIS_MAX_HPF = 18.0           # cap analysis at 18 hpf (pre-disintegration)
PEAK_WINDOW_HPF = (8.0, 16.0)     # meaningful biology window; phenotype counts any
                                  # pole present in this window.
...
PHENOTYPE_CONDITIONS = ["Pctrl", "P10ngml", "P30ngml", "P50ngml"]
...
def classify_mezzo_tracks(track_id_stack, masks_aligned, gfp_aligned, bg_means,
    ...
    for tid in all_ids:
        ...
        # Classification
        peak_intensity_for_class = float(peak_row["mean_int_bgsub"])
        is_big_pole = (peak_area >= AREA_POLE_PX)
        is_small_bright_pole = (peak_area >= SMALL_POLE_AREA_MIN
                                and peak_intensity_for_class >= SMALL_POLE_INTENSITY_MIN)
        if (duration >= POLE_PERSISTENCE_MIN
                and (is_big_pole or is_small_bright_pole)
                and peak_offset > PERIPHERAL_OFFSET_FRAC
                and independent):
            ctype = "pole"
        elif (duration >= OC_PERSISTENCE_MIN and peak_area >= AREA_OC_PX
              and (peak_offset <= PERIPHERAL_OFFSET_FRAC or not (is_big_pole or is_small_bright_pole))):
...
def mezzo_local_vs_surrounding(track_summary, masks, gfp, bg_means):
    ...
    surrounding_mean: mean bg-subtracted GFP in (mask minus disk)
...
def process_pescoid(cond, pid, global_thr, save_outputs=True,
    ...
    # Cap analysis at ANALYSIS_MAX_HPF (16 hpf) OR disintegration_frame, earlier wins.
    # Beyond 16 hpf the pescoids start disintegrating; that biology is not informative
    # for the multipolar phenotype.
    analysis_cap_frame = int(np.floor((ANALYSIS_MAX_HPF - HPF_START) / HPF_INTERVAL)) + 1
    T_eff = min(T, analysis_cap_frame)
    if disintegration_frame is not None and disintegration_frame > 0:
        T_eff = min(T_eff, int(disintegration_frame))
    if T_eff < T:
    ...
    # endstate = last 3 frames that actually have a mask (skips trailing blank frames)
    masked_frames = [t for t in range(T) if masks[t].any()]
    endstate_frames = masked_frames[-3:] if masked_frames else list(range(max(0, T - 3), T))
...
if __name__ == "__main__":
    ...
    p.add_argument("--sanity", action="store_true",
                   help="Run on S16/S31/S44 (one per dose group) for sanity check")
    ...
    args = p.parse_args()
    if args.sanity:
        run(pescoid_filter=["S16", "S31", "S44"])
    elif args.all:
        run()
    elif args.pids:
