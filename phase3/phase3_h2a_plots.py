...
Adapted from phase3_plots.py: new paths, S-numbered pescoids,
7 hpf start / 11.65 min interval, 7-16 hpf peak window.
...
Outputs (in Z:\\Megha_Kattimani\\Full_pipeline test\\mezzo_H2A_phase3\\):
...
OUT = Path(r"Z:\Megha_Kattimani\Full_pipeline test\mezzo_H2A_phase3")
PLOTS = OUT / "plots"
TABLES = OUT / "tables"
...
def load_perframe_metrics_from_phase1():
    ...
    GLOBAL_THR_BGSUB = 789.05  # established in Phase 2 (P_ctrl-only, p99, frames 0-10)
    rows = []
    for cond in CONDITIONS:
        pdir = PHASE1 / "per_pescoid" / cond
        if not pdir.exists():
