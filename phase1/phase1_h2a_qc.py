...
DATA_ROOT = Path(
    ...
    r"\260512_mezzo_H2A_3-5hpfActivin_obj10x_7hpf\TIFF"
)
OUT = Path(r"Z:\Megha_Kattimani\Full_pipeline test\mezzo_H2A_phase1")
PER = OUT / "per_pescoid"
QC = OUT / "qc_images"
for d in (OUT, PER, QC):
...
# Channel indices in the raw (T,Z,C,Y,X) stack
CH_GFP = 0   # mezzo-GFP
CH_H2A = 1   # H2A nuclei
CH_BF = 2    # brightfield
...
QC_DISINT_MIN_HPF = 9.0              # ignore area drops before this hpf (early growth)
...
def scene_condition(scene_num):
    """Contiguous-block condition layout (user-provided)."""
    if 1 <= scene_num <= 10:
        return "E_ctrl"
    if 11 <= scene_num <= 22:
        return "P_ctrl"
    if 23 <= scene_num <= 50:
        return "P_Activin_3-5hpf"
    return None
...
def process_pescoid(scene_num, cond, sample_path, unet, device):
    ...
    def _seg_with_guard(bf_img):
        """Segment one preprocessed BF image; discard background leaks."""
    ...
    aligned_h2a = np.zeros_like(h2a_raw)
    orient_aligned = []
    centroid_aligned = []
    centroid_drift_px = []
    for t in range(T):
        ...
        aligned_h2a[t] = rotate_centered(h2a_raw[t], ref_angle_deg, (cy, cx), order=1)
    ...
    tifffile.imwrite(str(pdir / "h2a_aligned.tif"), aligned_h2a.astype(np.float32))
